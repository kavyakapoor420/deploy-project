 ...
 */
const authenticateToken = (req: Request, res: Response, next: NextFunction): void => { // Explicitly set return type to void
 ...
 */
app.post('/comments', authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (parent_id) {
      ...
      if (parentComment && parentComment.user_id !== userId) {
        await client.query(
          'INSERT INTO notifications (user_id, comment_id, reply_id, message) VALUES ($1, $2, $3, $4)',
          [
 ...
 */
app.put('/comments/:id', authenticateToken, async (req: Request, res: Response) => {
 ...
 */
app.delete('/comments/:id', authenticateToken, async (req: Request, res: Response) => {
  const commentId = req.params.id;
  const userId = req.user?.id;
 ...
 */
app.put('/comments/:id/restore', authenticateToken, async (req: Request, res: Response) => {
  const commentId = req.params.id;
  const userId = req.user?.id;
